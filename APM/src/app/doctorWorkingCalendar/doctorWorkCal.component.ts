...
export class DoctorWorkCalendar  implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this._docWorkCalService.getVacations(parseInt(localStorage.getItem('user-id'))).subscribe(
      data=>{
          this.vacations = data;
          for(let i=0; i< this.vacations.length; i++){
            this.events = [...this.events,{
                title : 'Vacation',
                start : new Date(this.vacations[i].from),
                end: new Date(this.vacations[i].to),
                id : this.vacations[i].id,
                color: colors.blue
            }];
          }      
      }, 
